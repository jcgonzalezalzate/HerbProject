...
namespace HERBProject.WebUI.Helpers
{
    ...
    public class Log4netTraceListener : TraceListener
    {
        ...
        #region Variables
        private static readonly ILog _log = LogManager.GetLogger("HERBProject.WebUI");
        #endregion
            ...
            switch (eventType)
            {
                case TraceEventType.Error:
                         ...
                         (ActiveTraceLevel == TraceLevel.Verbose))
                    {
                        try
                        {
                            _log.Error(message + eventCache.Callstack.Substring(eventCache.Callstack.LastIndexOf("System.Diagnostics.Trace.TraceError(String message)")).Replace("System.Diagnostics.Trace.TraceError(String message)\r\n", ""));
                        }
                        catch (Exception)
                        {
                            _log.Error(message);
                ...
                case TraceEventType.Warning:
                         ...
                         (ActiveTraceLevel == TraceLevel.Verbose))
                    {
                        try
                        {
                            _log.Warn(message + eventCache.Callstack.Substring(eventCache.Callstack.LastIndexOf("System.Diagnostics.Trace.TraceError(String message)")).Replace("System.Diagnostics.Trace.TraceError(String message)\r\n", ""));
                        }
                        catch (Exception)
                        {
                            _log.Warn(message);
